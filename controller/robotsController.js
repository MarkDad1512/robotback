...
exports.move = (req, res) => {
 try{ let robotName = req.body.name;
  const robots = db.get("robots").value();
...
try {
  // check if req.body (robot name, title, ...) is empty? If empty, then respond with an error
  if (isEmpty(req.body)) {
      // resond with an error
